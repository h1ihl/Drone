...
const rows = [
  ...
  ['RC link', 'ESP32-C3 Super Mini, 4MB flash (onboard RX)', 1, 'RISC-V, 2.4GHz WiFi, custom firmware: ESP-NOW RX -> SBUS out (inverted UART) to FC', {cad:1.68}, 'AliExpress (Simple Robot Store)', 'https://www.aliexpress.com/item/1005007205044247.html', 'No', 'Yes', 'Recommended', 'Purchased', 'Manufacturer-specified (price/specs from listing, 590 reviews/10,000+ sold, one review confirms board-to-board wireless comms testing); a minority of reviews report WiFi not working out of the box, worth bench-testing WiFi/ESP-NOW range early (Test 5) rather than assuming it works'],
  ['RC link', 'ESP32-WROOM-32 DevKitC (handheld TX) + KY-023 dual-axis joystick x2 + momentary/toggle switch x2', 1, 'Dual-core, 2.4GHz WiFi, custom firmware: reads sticks/switches -> ESP-NOW out. 4 stick axes (throttle, yaw, pitch, roll) + 2 AUX switches (arm, payload release). Powered via USB power bank, already owned, not priced separately.', {cad:4.46}, 'AliExpress (DevKit C$1.40 + 2x joystick $0.60 USD + 2x switch ~$0.50 USD est.)', 'https://www.aliexpress.com/item/1005008806241325.html ; https://www.aliexpress.com/item/1882818018.html', 'No', 'Yes', 'Recommended', 'Purchased', 'Manufacturer-specified (DevKit price/specs from listing, 4.9 stars/32 reviews); joystick price from a real AliExpress listing found; switch price is a typical-component estimate, no specific listing checked'],
