...
| RC link — onboard RX | ESP32-C3 Super Mini, 4MB flash | RISC-V, 2.4GHz WiFi, custom firmware: ESP-NOW in -> SBUS out (inverted UART) to the FC; a minority of buyer reviews report WiFi not working out of the box, worth bench-testing early | C$1.68 | [AliExpress (Simple Robot Store)](https://www.aliexpress.com/item/1005007205044247.html) |
| RC link — handheld TX | ESP32-WROOM-32 DevKitC | Dual-core, 2.4GHz WiFi, custom firmware: reads sticks/switches -> ESP-NOW out | C$1.40 | [AliExpress](https://www.aliexpress.com/item/1005008806241325.html) |
...
**Total purchased (Recommended, brushless): see `../BOM/BOM.xlsx`, real-priced total is ~$112.42 CAD**, just above my original $100 CAD target. Real brushless FPV micro-motor pricing was initially found at ~$51 USD (~$71 CAD) for a matched 4-pack across a name-brand retailer (Pyrodrone/BetaFPV) and a generic AliExpress listing at nearly identical prices, which pushed the total to ~$155 CAD, documented honestly as a real cost floor at the time, not a shopping mistake. A later, more thorough AliExpress search (comparison-shopped against several other 1103/11000KV-class listings, verified for true per-motor price rather than a misleading multi-pack headline price) turned up the SPARKHOBBY XSPEED 1103 11000KV 4-pack at C$33.58 (~$8.40/motor) — same spec (1103, 11000KV, 1.5mm shaft, 2-3S rated) and explicitly compatible with the Gemfan/HQprop 65mm props already in this BOM, at under half the original motor cost. That single swap dropped the Recommended-tier total from ~$155 CAD to ~$117.95 CAD. Separately, the RX/TX ESP32 boards were also re-sourced to cheaper AliExpress listings (ESP32-C3 Super Mini at C$1.68, ESP32-WROOM-32 DevKitC at C$1.40, both verified against real listings with review history rather than a rough price estimate), dropping the combined RX+TX board cost from ~$11.67 CAD to ~$6.14 CAD and the Recommended-tier total further to ~$112.42 CAD. The Minimum tier uses the same RC link, so it dropped by the same ~$5.53 CAD, from ~$124.68 CAD to ~$119.15 CAD. Swapping the FlySky TX/RX pair for the custom ESP32 link is no longer just cost-neutral, it's now meaningfully cheaper (~$6.14 CAD combined for the RX/TX ESP32s, plus ~$3.06 CAD for the joysticks and switches, vs. the ~$13.89 CAD the FlySky receiver alone cost, see `../BOM/BOM.xlsx`) — though it remains a scope decision about demonstrating embedded programming first, not a budget decision.
